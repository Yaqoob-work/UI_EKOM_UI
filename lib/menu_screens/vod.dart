...
class _FocusableGridItemContentState extends State<FocusableGridItemContent> {
  bool isFocused = false;
  Color dominantColor = Colors.transparent; // Default border color
  ...
  // Fetch palette color using ColorUtils and update dominantColor
  Future<void> _loadPaletteColor() async {
    Color paletteColor = await ColorUtils.getPaletteColor(widget.content.banner);
    setState(() {
      dominantColor = paletteColor; // Update border color with fetched palette color
    });
  }
...
class _DetailsPageState extends State<DetailsPage> {
  ...
  Widget _buildMovieDetailsUI(BuildContext context, MovieDetailsApi movieDetails) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(movieDetails.name, style: TextStyle(color: Colors.white, fontSize: 20)),
          SizedBox(height: 10),
          Expanded(
  ...
  void _handleVideoError(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Something Went Wrong', style: TextStyle(fontSize: 20))),
    );
  }
}